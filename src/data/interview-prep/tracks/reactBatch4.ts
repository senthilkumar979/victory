...
export const REACT_BATCH_4: InterviewQuestion[] = [
  ...
  },
  {
    ...
    answer:
      'When mapping a list, the outermost element returned from the callback needs a `key`. If you return a `Fragment` as the outermost node, use `<Fragment key={id}>` rather than the empty shorthand `<>`, because the shorthand cannot accept props including `key`. This trips up developers new to React 16+ fragments. Keys on fragments identify list items for reconciliation like any element. Interview edge: multiple elements wrapped without an array may still need keys if using fragments in an array of siblings—structure keys at the array map level.',
    codeLanguage: 'tsx',
  },
  {
    id: 'r-089',
  ...
  },
  {
    ...
    answer:
      'Model state as `{ present, past, future }` arrays or structured commands. Actions push present into `past`, clear `future` on new edits, undo pops from `past` to `future`, redo reverses. Keep history bounded to avoid memory growth. For text editors, operational transforms or CRDTs may replace naive stacks—scope appropriately. Immutability makes history copies tractable with structural sharing or libraries like Immer. Interview: discuss performance for large documents—chunking history or storing diffs instead of full snapshots. Test edge cases: undo after concurrent edits.',
    codeLanguage: 'tsx',
  },
  {
    id: 'r-091',
  ...
  },
  {
    ...
    answer:
      'Tearing is when UI shows inconsistent values simultaneously—part of the screen reads one snapshot of external mutable state while another part reads a different snapshot mid-update. Concurrent rendering exacerbates this if external stores mutate during render. `useSyncExternalStore` prevents tearing for external stores by coordinating subscribe/read. Pure React state does not tear because updates are atomic per render. Interview: this is advanced; connect to Redux 5 using `useSyncExternalStore` internally. Misuse of refs to store cross-component mutable data without coordination can also cause visual inconsistency.',
    codeLanguage: 'tsx',
  },
  {
    id: 'r-094',
