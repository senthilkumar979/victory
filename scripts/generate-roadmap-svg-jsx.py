...
def emit_element(elem: ET.Element, parent: ET.Element | None, indent: int) -> list[str]:
    ...
    topic_subtopic_checkmark: list[str] = []
    if tag == "g" and dtype in ("topic", "subtopic") and node_id:
        for ch in elem:
            if ch.tag.split("}")[-1] != "rect":
                continue
            ra = dict(ch.attrib)
            fill = (ra.get("fill") or "").lower()
            stroke = (ra.get("stroke") or "").lower()
            if fill == "transparent" and stroke == "transparent":
                continue
            try:
                x = float(ra["x"])
                y = float(ra["y"])
    ...
    lines.extend(topic_subtopic_checkmark)
...
def main() -> None:
    ...
    ap.add_argument(
        "--standalone",
        action="store_true",
        help="Emit one component file with useCallback + RoadmapDetailsProps (ReactRoadmap-style).",
    )
    args = ap.parse_args()
    ...
    raw = re.sub(r"#2[Bb]78[Ee]4\b", "#cea500", raw)
